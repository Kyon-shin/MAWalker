...
public class GotoFloor {
	...
	public static boolean run() throws Exception {
		...
		try {
			response = Process.network.ConnectToServer(URL_GET_FLOOR, post,
					false);
		} catch (Exception ex) {
		...
		try {
			...
			Process.info.gold = Integer.parseInt(xpath.evaluate(
					"//your_data/gold", doc));
			...
			Process.info.guildteamname = xpath.evaluate(
					"//your_data/party_name", doc);
			...
			Process.info.exp = Integer.parseInt(xpath.evaluate(
					"//get_floor/next_exp", doc));
			String spec = xpath.evaluate(
					"//get_floor/special_item/before_count", doc);
			if (spec.length() != 0) {
